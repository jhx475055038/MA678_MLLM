...
```{r}
mod1<- glm(match~attr_o +sinc_o +intel_o +fun_o +amb_o +shar_o,data=dating,family=binomial)
summary(mod1)
coefplot(mod1)
...
```{r}
mod2 <- lmer(match~gender+partner+scale(attr_o) +scale(sinc_o) +scale(intel_o) +scale(fun_o) +scale(amb_o) +scale(shar_o)+(1|id),data=dating,family=binomial(link="logit"))
summary(mod2)
...
```{r}
mod3 <- glmer(match~gender+partner+scale(attr_o) +scale(sinc_o) +scale(intel_o) +scale(fun_o) +scale(amb_o) +scale(shar_o)+(1|id)+(1|idg),data=dating,family=binomial(link="logit"))
summary(mod3)
...
```{r}
mod4 <- glm(match~attr_o + sinc_o + intel_o + fun_o + amb_o + shar_o + factor(iid)-1,data=dating)
summary(mod4)
# According to the summary(mod4), we know that mod4 is varied by person
```

5. Fit a multilevel model, allowing the intercept and the coefficients for the 6 ratings to vary by the rater i.
```{r}
mod5 <- glmer(match~(1+attr_o+sinc_o+intel_o+fun_o+amb_o+shar_o|iid) + attr_o + sinc_o + intel_o + fun_o + amb_o + shar_o,data=dating)
summary(mod5)
```
